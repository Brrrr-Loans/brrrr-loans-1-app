...
import {
  ...
  getDealRoleTypes,
} from "@/app/(portal)/org/[clerk_org_id]/settings/policies/actions";
import {
  FEATURE_RESOURCES,
...
const SCOPE_SUBJECTS: Array<{ value: string; label: string; description: string }> = [
  { value: "active_org", label: "Active Organization", description: "auth_clerk_orgs.id of the logged-in org" },
  { value: "current_user_clerk", label: "Logged-in User (Clerk ID)", description: "auth_clerk_users.clerk_user_id from JWT" },
  { value: "current_user_pk", label: "Logged-in User (User ID)", description: "auth_clerk_users.id resolved from JWT" },
];
...
export default function OrgPolicyBuilder({
    ...
    startTransition(async () => {
      try {
        ...
        if (editingPolicyId) {
          ...
          if (current && !canMutatePolicy(current, "edit")) {
            setError(
              isGlobalPolicy(current)
                ? "Inherited global policies are read-only for this organization."
                : "This policy cannot be edited in the UI."
            );
            return;
          }
  ...
  async function handleToggleActive(id: string, isActive: boolean) {
    ...
    if (!canMutatePolicy(previous, "toggle")) {
      setError(
        isGlobalPolicy(previous)
          ? "Inherited global policies are read-only for this organization."
          : "Protected policies cannot be deactivated."
      );
      return;
    }
  ...
  async function handleDelete(id: string) {
    ...
    setStatus(null);
    const previous = policies.find((p) => p.id === id);
    if (previous && !canMutatePolicy(previous, "archive")) {
      setError(
        isGlobalPolicy(previous)
          ? "Inherited global policies are read-only for this organization."
          : "This policy cannot be archived from the UI."
      );
      return;
    }
    startTransition(async () => {
  ...
  const editingCompiledConfig = editingPolicy?.compiled_config as
    ...
    | undefined;
  const isMultiRulePolicy = editingPolicy
    ? isMultiRuleSubject(editingPolicy)
    : (editingCompiledConfig?.rules?.length ?? 0) > 1;
  const isProtectedPolicy = editingPolicy
    ? isProtectedSubject(editingPolicy)
    : false;
  const isInheritedGlobal = editingPolicy
    ? isGlobalPolicy(editingPolicy)
    : false;
  const isReadOnlyEdit =
    isProtectedPolicy || isMultiRulePolicy || isInheritedGlobal;

  const policyFormBody = (
    <div className="space-y-6">

      {/* Inherited global — read-only notice */}
      {editingPolicyId && isInheritedGlobal && (
        <div className="flex items-start gap-3 rounded-lg border border-sky-500/40 bg-sky-500/10 px-4 py-3 text-sm">
          <Lock className="h-4 w-4 text-sky-500 mt-0.5 shrink-0" />
          <div>
            <p className="font-semibold text-sky-700 dark:text-sky-400">Inherited global policy</p>
            <p className="text-muted-foreground mt-0.5">
              Policies with no organization owner are inherited by every org and are
              read-only here. They cannot be edited, toggled, or archived in this catalogue.
            </p>
          </div>
        </div>
      )}
      ...
      {/* Multi-rule policy — view-only */}
      {editingPolicyId && isMultiRulePolicy && !isProtectedPolicy && !isInheritedGlobal && (
        <div className="flex items-start gap-3 rounded-lg border border-blue-500/40 bg-blue-500/10 px-4 py-3 text-sm">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-0.5">
              This policy contains multiple rule groups. The form shows the{" "}
              <strong>first rule</strong> for reference only. Multi-rule policies are
              view-only in the UI — use a SQL migration to edit them safely.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap mb-6">
        ...
        <p className="basis-full text-xs text-muted-foreground">
          Effect applies to every fan-out row this form saves. If both ALLOW and
          DENY policies match the same request, <strong>DENY always wins</strong>.
        </p>
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <Button
          ...
          title={
            isInheritedGlobal
              ? "Inherited global policies are read-only"
              : isProtectedPolicy
    ...
    <div className="space-y-8 pb-12">
      <Card className="bg-transparent shadow-none">
        <CardHeader>
          ...
          <CardDescription>
            Define who can access what using conditions and actions. When an
            ALLOW and a DENY policy both match, <strong>DENY always wins</strong>.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Sheet open={!!editingPolicyId} onOpenChange={(open) => { if (!open) resetForm(); }}>
        <SheetContent side="right" className="sm:max-w-2xl w-full overflow-y-auto">
          <SheetHeader>
            <SheetTitle>
              {isReadOnlyEdit ? "View Access Policy" : "Edit Access Policy"}
            </SheetTitle>
            <SheetDescription>
              {isReadOnlyEdit
                ? "This policy is view-only in the builder."
...
function resolveOrgLabel(policy: OrgPolicyRow, orgDisplayName: string): string {
  return isGlobalPolicy(policy) ? "Inherited (global)" : orgDisplayName;
}
...
function PolicyTableRow({
  ...
  const isProtected = isProtectedSubject(policy);
  const isInherited = isGlobalPolicy(policy);
  const rowCompiled = policy.compiled_config as { rules?: unknown[] } | undefined;
  const isMultiRule = isMultiRuleSubject(policy);
  const mutationLocked = !canMutatePolicy(policy, "edit");
      ...
      {visibleColumns.isActive && (
        <td className={tdCls}>
          <ActiveToggleBadge
            ...
            isProtected={isProtected || isInherited}
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuGroup>
              ...
              <DropdownMenuItem
                ...
                disabled={mutationLocked || isInherited}
              >
      ...
      {columns.map((col) => {
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {col.items.map((p) => {
              const isProtected = isProtectedSubject(p);
              const isInherited = isGlobalPolicy(p);
              return (
                <div
                  key={p.id}
                  onClick={() => onEdit(p)}
